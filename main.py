...
פיקוס כינורי: להשקות כש-50% יבש, שונא הזזה | מונסטרה: עמוד מוס, לעלים גדולים=אור חזק עקיף+השרשת שורשי אוויר (מים/לקה)→שורשים פעילים→יותר קליטה | סחלב: טבילה 10 דק' פעם בשבוע, שורשים ירוקים=לא להשקות, חובה ניקוז
שפלרה: אור חזק מסונן, להשקות כשהשכבה העליונה יבשה, מצע אוורירי, אוהבת צפיפות שורשים בכלי | כוסברה: אוהבת קרירות, 4-6 ש' שמש, אדמה לחה לא בוצית, לא לתת לפרוח
נענע: 3-5 ש' שמש ישירה, אדמה לחה תמיד, כל קטימה מפצלת לשיח, שורשים פולשניים=עציץ נפרד, מהסופר=להשריש ענפים במים ואז לאדמה
סוקולנט: 4-6 ש' שמש ישירה (מתארך/משתטח=חסר אור), להשקות רק כשכל האדמה יבשה לגמרי (עודף מים=אויב מס' 1, בחורף כמעט לא), מצע מנקז אדמה+פרלייט/טוף חצי-חצי + חור ניקוז | הכפלת סוקולנט: לתלוש עלה שלם עם בסיס שלם, על אדמה יבשה, ריסוס כל כמה ימים → שורשים+ניצן תוך 2-3 שבועות
ורדים: שמש ישירה 6 ש', השקיה לעומק רק כשהשכבה העליונה יבשה, מצע מנקז+חור ניקוז, דישון לצמחים פורחים כל 2-3 שבועות | פורחים על ענפים חדשים→גיזום חזק בסוף חורף + הסרת כל פרח שנבל = יותר פריחה | להשקות את האדמה ולא את העלים (עלים רטובים=כתמים שחורים/קימחון)
...
══ נושאים עם סרטונים ══
ורדים (שיח ורדים פורח מגיזום), נענע (שיח שופע, זחל המודד, שלוחות, השרשה במים מנענע מהסופר), סוקולנט (הכפלה מעלים + טיפול), פיקוס כינורי (השרשה + טיפול), אלוקסיה / אוזן פיל (פקעות, הפרדת ניצרים, הברכת אוויר, לקה ומוס, אקריות, הצלה), פילודנדרון סקנדס, בזיליקום (השרשה במים + טיפול לשיח שופע), כנימה קמחית, כנימות עלה, זחל המודד, מונסטרה (הגדלת עלים עם שורשי אוויר, עלים ענקיים מחוררים, ייחור), קולאוס (השרשה + גידול), סחלב, דשן מבננה, ביצן (הצלה + ייחור), השרשת/הברכת אוויר, שפלרה מקרינה (השרשת ענף מהרחוב + טיפול), כוסברה (שמירה טרייה במקרר), פוטוס (השרשה מ-40 ייחורים), כלי השרשה עיצוביים (אצטרובל, שרוך וצנצנת), פיטנה / פלומריה (השרשת ענף מהרחוב), בצל ירוק מהסופר, גת, זרעי צ׳יה / נבטים, השרשת צמחי תבלין במים, השרשת צמחי בית במים (בגוניה, לאקי במבוק, סינגוניום, דרצנה), פרחים בזר (שמירה עם סוכר ואקונומיקה), זיהוי מזיקים במשתלה, כדורי לקה (מהי לקה, לקה ומוס), דישון נכון (קפה זה לא דשן), קליפות ביצים כדשן, ירקה / צמח העכביש (ריבוי), דרצנה (הצלה), אקריות, ניצנית (הפרחת בצל מחדש), תה מגידול ביתי, השקיה בחורף, תאורה בחורף, שכפול במבוק, לואיזה (הצלה בגיזום), צמחים לסלון מואר
